...
> 让 AI 帮你从多个视角思考 · [Web 生成器](https://roundtable.1dian.site/) · CC BY-SA 4.0 · [GitHub](https://github.com/FartherQuant/roundtable) · [English](README.en.md)
...
### 方式一：Web 生成器（推荐，零门槛）
访问 [https://roundtable.1dian.site/](https://roundtable.1dian.site/)，输入议题、勾选人格、复制 prompt、粘贴到任意 LLM。
...
如果不想手工编写 YAML，可以使用 [templates/persona-generator.md](templates/persona-generator.md) 让 LLM 帮你生成：

1. 打开 [templates/persona-generator.md](templates/persona-generator.md)
2. 将模板中的 `{人物名}` 替换为你想加入的人物（如 `稻盛和夫` / `本·霍洛维茨` / `香帅`）
3. 复制全文粘贴到 ChatGPT / Claude / 任何 LLM
4. 约 30 秒后即可得到符合 `persona-schema.yaml` 规范的 YAML 配方
5. 把输出保存为 `personas/<英文名>.yaml`，即可在 Web 生成器与命令行中调用

设计要求（生成器会强制约束）：

- **speaking_style** 必须体现独特的语言指纹，让人一眼能认出是谁
- **stance_anchors** 2-5 条，动词开头，定义这个人格切入问题的独特入口
- **honesty_rules** 必须诚实声明方法的边界
- **anti_pattern** 必须指向核心优势的过度延伸
